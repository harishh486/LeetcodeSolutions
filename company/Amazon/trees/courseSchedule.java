class Solution {
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        return counter == numCourses; // wehich means we already completed all course
...
T: O(∣E∣+∣V∣) where |V|is the number of courses, and |E| is the number of dependencies.

it would take us ∣E∣ time complexity to build a graph in the first step.

we would visit each vertex and each edge once and only once in the worst O(∣E∣+∣V∣).

S: O(∣E∣+∣V∣)

raph data structure in the algorithm, which would consume∣E∣+∣V∣ space.
 we use a array to keep track of the courses that have no prerequisite, and the size of the array would be bounded by ∣V∣.
overall space complexity of the algorithm would be O(∣E∣+2⋅∣V∣)=  O(∣E∣+∣V∣).